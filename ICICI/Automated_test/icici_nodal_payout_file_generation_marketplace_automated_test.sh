...
left join merchant_txp_meta_data mtmd on (t.id=mtmd.parent_id) left join merchant mm on(wapg.mid=mm.mid) where (wapg.mid in (select mid from icici_payout_merchants  where isPayoutEnabled=1) or if(wapg.smid is not NULL, wapg.smid in (select mid from icici_payout_merchants  where isPayoutEnabled=1),(wapg.mid in (select mid from icici_payout_merchants  where isPayoutEnabled=1)))) and wapg.payoutbatchid like concat('%',date_format(now(),'%Y%m%d'),'%') and wapg.payoutbatchid IN (SELECT DISTINCT( batch_id ) FROM  settlement_wapg WHERE  (status = 'automated_success' OR status = 'automated_failure' OR status = 'automated_pending')  and created_at >= Curdate()) and ismarketplace ='y' and paymentType not in (2,3,4) and wapg.updatedat>=date(now()) and wapg.settlementdate>=date(now())
union all
...
left join merchant_txp_meta_data mtmd on (t.id=mtmd.parent_id)  left join merchant mm on(wapg.mid=mm.mid) where (wapg.mid in (select mid from icici_payout_merchants  where isPayoutEnabled=1) or if(wapg.smid is not NULL, wapg.smid in (select mid from icici_payout_merchants  where isPayoutEnabled=1),(wapg.mid in (select mid from icici_payout_merchants  where isPayoutEnabled=1)))) and wapg.refundbatchid like concat('%',date_format(now(),'%Y%m%d'),'%') and wapg.refundbatchid IN (SELECT DISTINCT( batch_id ) FROM  settlement_wapg WHERE (status = 'automated_success' OR status = 'automated_failure' OR status = 'automated_pending')   and created_at >= Curdate()) and  ismarketplace='y' and paymentType not in (2,3,4) and wapg.updatedat>=date(now()) and wapg.settlementdate>=date(now())  order by 1"  | $MYSQL --login-path=mobinewcronmaster_RDS01 -D $DB | sed 's/\t/","/g;s/^/"/;s/$/"/;s/\n//g' >> /tmp/cronreports/working/WAPG_ICICI_Working_file_Mplace_$CURRENTDATE.csv
TIMESTAMP=`date "+%Y-%m-%d %H-%M-%S"`
