...
where sw.merchant_id = m.mid and sw.created_at>=date(now()) and (sw.status = 'automated_success' OR sw.status = 'automated_failure' OR sw.status = 'automated_pending')
