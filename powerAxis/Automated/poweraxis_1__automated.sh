...
put Tables_timestamp_data_automated.csv
...
"| $MYSQL --login-path=mobinewcronmaster_RDS01 -D $DB | sed 's/\t/","/g;s/^/"/;s/$/"/;s/\n//g' > /data/cronreport-payout/AXIS_ESCROW_NMP_PAYOUT_FILE_automated.csv
...
hash
lcd /data/cronreport-payout/
put AXIS_ESCROW_NMP_PAYOUT_FILE_automated.csv
...
where ks.merchant_id = m.mid
        ...
        and mid in (
                ...
                from merchant_payout_config
                where power_access_file = 1
        );"| $MYSQL --login-path=mobinewcronmaster_RDS01 -D $DB | sed 's/\t/","/g;s/^/"/;s/$/"/;s/\n//g' > /data/cronreport-payout/AXIS_ESCROW_MP_MERCHANT_PAYOUT_FILE_automated.csv
...
hash
lcd /data/cronreport-payout/
put AXIS_ESCROW_MP_MERCHANT_PAYOUT_FILE_automated.csv
...
"| $MYSQL --login-path=mobinewcronmaster_RDS01 -D $DB | sed 's/\t/","/g;s/^/"/;s/$/"/;s/\n//g' > /data/cronreport-payout/AXIS_ESCROW_MP_SUBMERCHANT_PAYOUT_FILE_automated.csv
...
hash
lcd /data/cronreport-payout/
put AXIS_ESCROW_MP_SUBMERCHANT_PAYOUT_FILE_automated.csv
...
group by s.parentmid,
        batch_id;
"| $MYSQL --login-path=mobinewcronmaster_RDS01 -D $DB | sed 's/\t/","/g;s/^/"/;s/$/"/;s/\n//g' > /data/cronreport-payout/AXIS_NODAL_MP_PAYOUT_FILE_automated.csv
...
hash
lcd /data/cronreport-payout/
put AXIS_NODAL_MP_PAYOUT_FILE_automated.csv
...
where sw.merchant_id = m.mid
        ...
        and sw.merchant_id in (
                ...
                from merchant_payout_config
                where power_access_file = 1
        );"| $MYSQL --login-path=mobinewcronmaster_RDS01 -D $DB | sed 's/\t/","/g;s/^/"/;s/$/"/;s/\n//g' > /data/cronreport-payout/AXIS_NODAL_NMP_PAYOUT_FILE_automated.csv
...
hash
lcd /data/cronreport-payout/
put AXIS_NODAL_NMP_PAYOUT_FILE_automated.csv
...
hash
lcd /data/cronreport-payout/
put AXIS_NODAL_NMP_WORKING_FILE_automated.csv
...
WHERE  w.settlementdate >= Date(Now())
       ...
       AND t.statecode between 28 and 68 and t.updatedat>=date(now()) AND (w.payoutbatchid, t.mid) IN (SELECT batch_id, merchant_id
...
hash
lcd /data/cronreport-payout/
put AXIS_ESCROW_NMP_WORKING_FILE_automated.csv
